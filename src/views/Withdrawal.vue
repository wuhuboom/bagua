  ...
  <div class="c-page bg-grey">
    ...
    <template>
      <div class="bd-box">
        ...
        <div
          class="lists"
          v-if="!GOpayCard.id && typeList.includes(4)"
          @click="$router.push('/bindGOpay')"
        >
          <div class="left">
            <van-icon name="plus" class="plus-icon" />
            <div class="name">绑定GOpay钱包</div>
          </div>
          <div class="right">
            <van-icon name="arrow" class="arrow-icon" />
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      iconList: [
        ...
        {
          icon: require("@/assets/img/Recharge/Gopay.png"),
          name: "GOpay钱包",
          ctype: 4,
        },
      ],
  ...
  computed: {
    ...
    bankTxt() {
      //0支付宝 1微信 2银行卡 3USDT
      const arr = ["支付宝", "微信", "银行卡", "USDT地址", "GOpay钱包"];
    ...
    GOpayCard() {
      return this.Cards.find((v) => +v.ctype === 4) || {};
    },
    usdtCard() {
  ...
  methods: {
    ...
    async ajaxBindCard() {
      ...
      if (err) return
      if (res.code == "103" && res.data[0].msgKey == "outWiTime") {
            let timerTxt = res.data[0].time;
            this.$toast.clear();
            Dialog.confirm({
              title: "提现提醒",
              message: `<span style="font-weight: bold;">无法提现, 请在规定时间 </span>
       <span> <span style="color: red; ">(${timerTxt}) </span><span>内提现</span></span>
      `,
              confirmButtonColor: "#3291FF",
              className: "custom-dialog",
              dangerouslyUseHTMLString: true, // 允许 HTML 解析
              // message: this.$t("backapi." + res.data[0].msgKey, { timeMsg: timerTxt }),
            })
              .then(() => {
                // on confirm
              })
            ...
            return false;
        } else if (res.code == "103" && res.data[0].msgKey == "withdrawalLimit") {
            this.$toast.clear();
            Dialog.confirm({
              title: "提现提醒",
              message: res.data[0].withdrawalLimitMsg,
              confirmButtonColor: "#3291FF",
              className: "custom-dialog",
              // dangerouslyUseHTMLString: true, // 允许 HTML 解析
            })
...
/deep/ .custom-dialog .van-dialog__title {
  color: blue;
  font-weight: bold;
  font-size: 24px;
  text-align: center;
