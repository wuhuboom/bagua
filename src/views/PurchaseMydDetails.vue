        ...
        <template v-if="showContent(detail.visibility)">
          ...
          >
            <li class="align-center p-b-8">
              ...
              ><span class="pink x-auto">{{ item | modifyString }}</span>
            </li>
...
export default {
  ...
  filters: {
    modifyString: function (str) {
      ...
      // console.log("111", item?.b?.replace(/,/g, "").replace(/\|/g, " | "));
      let strE = str?.b;
      if (strE?.includes("|")) {
        return strE?.replace(/,/g, "").replace(/\|/g, " | ").replace(/ /g, "");
      } else {
        console.log("strE?.replace(/,/g, " | ")", strE?.replace(/,/g, "|"));
        return strE?.replace(/,/g, "|").replace(/ /g, "");
      }
    },
  },
  computed: {
    ...
    isMe() {
      ...
      console.log("this.detail?.playerId", this.detail?.playerId);
  ...
  methods: {
    ...
    showContent(v) {
      ...
      if (v === 2) {
        return !this.detail.betCode ? this.isMe : true;
      }
      if (v === 3) {
        return this.isMe;
      }
